...
def _print_usage(console: Console | None = None) -> None:
    out = console or create_console()
    out.print("vk_uploader — upload YouTube videos to VK")
    out.print()
    out.print("Usage:")
    out.print("  vk_uploader setup               Interactive configuration wizard")
    out.print("  vk_uploader <youtube_url> [key=value ...]")
    out.print("  vk_uploader links_file=<path> [key=value ...]")
    out.print()
    out.print("Batch mode (links_file):")
    out.print("  Upload multiple videos from a text file, one URL per line.")
    out.print("  Per-line overrides use the same key=value syntax.")
    out.print("  Precedence: config.yaml < command-level overrides < line-level overrides.")
    out.print()
    out.print("  File format:")
    out.print("    <youtube_url> [key=value ...]")
    out.print("    ylink=<youtube_url> [key=value ...]")
    out.print("    # comments")
    out.print()
    out.print("  Example links.txt:")
    out.print("    https://www.youtube.com/watch?v=DsLQptIzUuM subtitles=true")
    out.print("    https://www.youtube.com/watch?v=1f5gEQHy2cg wallpost=true")
    out.print("    ylink=https://www.youtube.com/watch?v=abc title=\"Custom Title\"")
    out.print()
    out.print("Supported options:")
    out.print("  ylink=<url>              YouTube video URL (alternative to positional)")
    out.print("  thumbnail=true|false     Enable/disable YouTube thumbnail upload")
    out.print("  publish_delay_hours=<n>  Hours to delay video publication (default: 24)")
    out.print("  output_dir=<path>        Download directory (default: ~/Downloads)")
    out.print("  video_format=<fmt>       yt-dlp format string (default: bv*+ba[ext=m4a]/bv*+ba/b)")
    out.print("  token=<str>              VK access token (override config)")
    out.print("  group_id=<str>           VK group/community ID")
    out.print("  wallpost=true|false      Publish to community wall (default: false)")
    out.print("  translation=true|false   Translate title/description (default: false)")
    out.print("  subtitles=true|false     Download and translate subtitles (default: false)")
    out.print("  lang=<code>              Target language for translation/subtitles")
    out.print("  album=true|<name>        Add video to album (interactive or by name)")
    out.print("  title=<str>              Video title (default: from YouTube)")
    out.print("  description=<str>        Video description (default: from YouTube)")
    out.print(
        "  cleanup_after_upload=true|false"
        "  Remove local files after successful upload (default: false)"
    )
    out.print()
    out.print("Config file: ~/.config/vk_uploader/config.yaml")
    out.print()
    out.print("Supported browsers for cookies (yt-dlp --cookies-from-browser):")
    out.print("  firefox, chrome, chromium, brave, edge, opera, vivaldi")
    out.print()
    out.print("Use cookies_from_browser=<browser> to avoid YouTube bot detection.")
